...
export default function ExportPage({
  ...
  >(null);
  const [initiatorSatisfiedAt, setInitiatorSatisfiedAt] = useState<string | null>(null);
  const [responderSatisfiedAt, setResponderSatisfiedAt] = useState<string | null>(null);
  const [autoGenerateDueAt, setAutoGenerateDueAt] = useState<string | null>(null);
  ...
  const viewerSatisfied =
    viewerRole === "initiator" ? Boolean(initiatorSatisfiedAt) : Boolean(responderSatisfiedAt);
  const otherSatisfied =
    viewerRole === "initiator" ? Boolean(responderSatisfiedAt) : Boolean(initiatorSatisfiedAt);
  ...
  const loadDownloadStatus = useCallback(async () => {
    ...
    const payload = await parseJsonBody<DownloadPayload>(response);
  ...
  useEffect(() => {
    ...
    async function loadPageData() {
      try {
        ...
        const casePayload = (await caseResponse.json()) as CasePayload;
  ...
  async function handleMarkSatisfied() {
    ...
    try {
      const response = await fetchApi(`/api/cases/${caseId}/completion`, locale, {
        method: "PATCH",
      });

      const payload = (await response.json()) as {
        error?: string;
        case?: CasePayload["case"];
      };
  ...
  return (
    <main className=" px-5 py-6">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {!isSingleParty && !exportUnlocked ? (
          <Card className="app-panel border-brand/15">
            <CardContent className="space-y-4 p-6">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div className="app-note bg-surface-soft px-4 py-3 text-sm">
                  ...
                  <p className="mt-2 font-medium text-ink">
                    {viewerSatisfied ? t("export.confirmed") : t("export.awaitingYourConfirmation")}
                  </p>
                </div>
                <div className="app-note bg-surface-soft px-4 py-3 text-sm">
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-ink-soft">
                  ...
                  <p className="mt-2 font-medium text-ink">
                    {otherSatisfied ? t("export.confirmed") : t("export.awaitingOtherConfirmation")}
                  </p>
                </div>
              </div>
              ...
              {autoGenerateDueAt ? (
                <p className="text-sm leading-6 text-ink-soft">
                  {t("export.autoGenerateDue", { date: new Date(autoGenerateDueAt).toLocaleDateString(locale) })}
                </p>
              ) : null}
        ...
        ) : !hasReadyExport && !exportUnlocked ? null : !hasReadyExport && !canPurchaseExport ? (
          <Card className="app-panel">
